...
class SimonGame:
    ...
    def apretar_boton(self, color):
        ...
        if self.dificultad == 'Super Experto':
            if len(self.secuencia_jugador) == 2:
                # Comparar las secuencias sin importar el orden
                if set(self.secuencia_jugador) == set(self.secuencia[-1]):
            ...
            if self.secuencia_jugador == [col for seq in self.secuencia for col in seq][:len(self.secuencia_jugador)]:
                if len(self.secuencia_jugador) == len([col for seq in self.secuencia for col in seq]):
                    self.deshabilitar_botones()
                    self.app.after(1000 - self.timer_secuencia, self.rondaSiguiente)
                    self.puntaje += 1
                    self.label_puntaje.config(text=self.puntaje)
    ...
    def iluminar_boton(self, colores):
        for color in colores:
            botonColor = self.botones[color]
            color_original = botonColor['bg']
            botonColor['bg'] = 'white'
            self.app.after(500 - self.timer_dificultad, lambda col=color: self.botones[col].config(bg=color_original))
    ...
    def rondaSiguiente(self):
        ...
        self.secuencia_jugador = []
        if self.dificultad == 'Super Experto':
            colores = random.sample(self.colores, 2)
            self.secuencia.append(colores)
        else:
            color = random.choice(self.colores)
            self.secuencia.append([color])

    def mostrar_secuencia(self):
        for index, colores in enumerate(self.secuencia):
            self.app.after((1000 * index) - self.timer_secuencia, lambda cols=colores: self.iluminar_boton(cols))
    ...
    def menu(self):
        ...
        file_menu.add_command(label="Salir", command=self.app.destroy)
    ...
    def ver_puntajes(self):
        ...
        boton_cerrar = tk.Button(galeriaPuntaje, text="Cerrar", command=galeriaPuntaje.destroy)
