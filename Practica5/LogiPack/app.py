    ...
    if request.method == 'POST':
        ...
        if request.form['texto_dni'] and request.form['texto_nro_repartidor']:
            ...
            if tipo_usuario == None:
                ...
                repartidor = Repartidor.query.filter_by(numero=nro_repartidor).first()
                return render_template('bienvenidaRepartidor.html', datos = datosform,hora=datetime.now().hour,repartidor=repartidor)
...
@app.route('/buscar_paquete/<int:repartidor_id>')
def buscar_paquete(repartidor_id):
    
    return render_template('repartidorBuscarPaquete.html',repartidor_id = repartidor_id)

@app.route('/repartidor_buscar_paquete/<int:repartidor_id>')
def repartidor_buscar_paquete(repartidor_id):
    
    nro_envio = request.form['nro_envio']
    paquetes = Paquete.query.filter_by(numeroenvio = nro_envio,entregado=False,idrepartidor=repartidor_id).all()
    return render_template('despachanteRegistrarPaquete.html',paquetes=paquetes, repartidor_id=repartidor_id)

@app.route('/repartidor_registrar_paquete', methods=['GET', 'POST'])
def repartidor_registrar_paquete():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            entregado2 = request.form.getlist('entregado')
            ...
            if entregado:
                ...
                if paquete:
                    ...
                    return render_template('repartidorRegistrarPaquete.html',band = True,repartidor_id = repartidor_id)
            
            else:
               
                return render_template('repartidorRegistrarPaquete.html',band = False, error="No se seleccionaron transportes.",repartidor_id=repartidor_id)
        
        except Exception as e:
           
            #transportes = Transporte.query.filter_by(fechahorallegada=None).all()
            return render_template('repartidorRegistrarPaquete.html', band = False,error=str(e),repartidor_id = repartidor_id)    
